...
<table id="register_family_members" border="0" cellspacing="2" cellpadding="4" width="600">
	<tr class="datatableheaderv2">
		<th class="datatableheaderv2" colspan=6" align="left">Ingrese la información de todos sus familiares y dependientes.</th>
	</tr>
	...
	<tr>
		...
		<td>
			<select id="miembro_<%= size+1 %>"  name="miembro_<%= size+1 %>"  size="1" required="required">
				...
				<option value="Hijo" <% if ( db.getRow(size).get(2).equals("Hijo")) { %> selected <% } %>>Hijo/a</option>
				<option value="Hermano" <% if ( db.getRow(size).get(2).equals("Hermano")) { %> selected <% } %>>Hermano/a)</option>
	    ...
	    <td><input type="text"  name="miembro_edad_<%= size+1 %>" value="<%= db.getRow(size).get(4) %>" required="required"></td>
