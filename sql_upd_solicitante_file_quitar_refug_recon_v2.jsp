...
<%
	...
	params.add("Subió razon de quitar refugiado reconocido de solicitante.");
...
<%
	...
	String mQuery = "update declaracion_jurada set " + 
					"adm_estatus_lkup = ? , " +
					"razon_de_quitar_refug_recon = ? , " +
					"fecha_de_quitar_refug_recon = ? , " +
					"quitar_refug_recon = ? , " + 
					"last_user_id = ? , " + 
					"last_mod_tmstmp = now()  " + 
					  "where id = ?";
	...
	String mFecha_de_quitar_refug_recon = null;
	String mRazon_de_quitar_refug_recon = null;
      ...
      try{ 
         ...
         while ( i.hasNext () ) 
         {
            ...
            if ( !fi.isFormField () )	
            {
				...
				} else if (fieldName.equals("fecha_de_quitar_refug_recon")) {
				 	mFecha_de_quitar_refug_recon = fi.getString();
				} else if (fieldName.equals("razon_de_quitar_refug_recon")) {
				 	mRazon_de_quitar_refug_recon = fi.getString();
				}  
            }
         }
...
<%
	...
	try{
		if (session.getAttribute("email") != null) {
			...
			prest.setString(1,mRazon_de_quitar_refug_recon);
			prest.setString(2,mRazon_de_quitar_refug_recon);
			prest.setString(3,mFecha_de_quitar_refug_recon);

			FileInputStream fs = new FileInputStream(file);
			prest.setBinaryStream(4,fs,fs.available());	
			prest.setInt(5,Integer.parseInt(mLast_user_id));
			prest.setInt(6,Integer.parseInt(mSolicitante_id));
